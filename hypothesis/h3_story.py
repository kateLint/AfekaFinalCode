...
# === Extract Readability Metrics from 'Story Analysis' ===
readability_keys = [
    "Flesch Reading Ease",
...
# Add flat readability columns
for key in readability_keys:
    colname = f"story_{key.lower().replace(' ', '_').replace('-', '_')}"
    df[colname] = df["Story Analysis"].apply(
        lambda x: x.get("Readability Scores", {}).get(key, np.nan) if isinstance(x, dict) else np.nan
    )

# Add lexical diversity & sentence length
df["story_lexical_diversity"] = df["Story Analysis"].apply(
    lambda x: x.get("Lexical Diversity", np.nan) if isinstance(x, dict) else np.nan
)
df["story_avg_sentence_length"] = df["Story Analysis"].apply(
...
# Filter only numeric readability features
readability_cols = [col for col in df.columns if col.startswith("story_") and pd.api.types.is_numeric_dtype(df[col])]
df = df[readability_cols + ["success"]].dropna()

X = df[readability_cols]
y = df["success"]
...
print("\n==== T-Test Results (Success vs Failed) ====")
print(ttest_df.to_string(index=False))

print(f"\nMean AUC: {auc_scores.mean():.3f} ± {auc_scores.std():.3f}")
